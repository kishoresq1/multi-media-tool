...
from app.collectors.vendor_advisory import VendorAdvisoryFetcher
...
from app.config.vendor_advisories import SOURCE_VENDOR_MAP, VENDOR_ADVISORY_SOURCE_IDS
...
from app.services.advisory_scorer import AdvisoryScorer

logger = logging.getLogger(__name__)

class AdvisorySearchService:
    """
    Search official vendor advisories (MSRC, Cisco, Fortinet, etc.)
    using vendor + vulnerability keywords via RSS / API / HTML scraper.
    """

    def __init__(self) -> None:
        self.fetcher = VendorAdvisoryFetcher()
        self.scorer = AdvisoryScorer()
    ...
    async def search_and_store(
        ...
        # Fetch all recent items from feeds/APIs; filter by keywords after collection
        fetch_terms = ["*"]
        source_ids = request.source_ids or VENDOR_ADVISORY_SOURCE_IDS
        sources = [
            SOURCE_REGISTRY[sid]
        ...
        tasks = [
            self.fetcher.fetch(source, fetch_terms, request.lookback_days)
        ...
        for source, result in zip(sources, results):
            ...
            matched = [
                ...
                if self._matches_advisory(s, source.id, request.vendors, request.lookback_days)
            ]
            source_stats[source.id] = {
                "name": source.name,
                "found": len(matched),
        ...
        saved: list[AdvisoryIntelResponse] = []
        min_score = normalize_min_score(request.min_confidence, default=40.0)
        ...
        for signal in all_signals:
            ...
            vendor_label = SOURCE_VENDOR_MAP.get(signal.source_id, [None])[0]
            ...
            if row:
                ...
                saved.append(advisory_to_response(row))
        ...
        return AdvisorySearchResponse(
            ...
            advisories_found=len(all_signals),
            advisories_saved=len(saved),
            total_in_database=total_in_db,
            started_at=started_at,
            completed_at=completed_at,
            duration_seconds=round((completed_at - started_at).total_seconds(), 2),
            advisories=saved[: request.result_limit],
        )
    ...
    def _matches_advisory(
        ...
        source_vendors = SOURCE_VENDOR_MAP.get(source_id, [])
        if requested_vendors:
            req_lower = [v.lower() for v in requested_vendors]
            vendor_match = (
                any(v.lower() in text for v in requested_vendors)
                or any(sv.lower() in req_lower for sv in source_vendors)
                or any(sv.lower() in text for sv in source_vendors if sv.lower() in req_lower)
            )
        else:
            vendor_match = True
...
def advisory_to_response(row) -> AdvisoryIntelResponse:
    return AdvisoryIntelResponse(
        id=row.id,
