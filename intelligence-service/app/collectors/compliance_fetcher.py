...
from app.config.compliance_sources import COMPLIANCE_FEED_OVERRIDES
...
class ComplianceFetcher:
    """Fetch compliance intel via RSS with HTML scraper fallback."""
    ...
    async def fetch(
        ...
        alt_feed = COMPLIANCE_FEED_OVERRIDES.get(source.id)
    ...
    def _prepare_source(self, source: SourceConfig) -> SourceConfig:
        feed = source.feed_url or COMPLIANCE_FEED_OVERRIDES.get(source.id)
        if feed:
    ...
    async def _safe_collect(
        ...
        try:
            ...
            logger.warning("Compliance collector %s failed: %s", collector.source.id, exc)
