...
from app.config.company_breach_sources import BREACH_FEED_OVERRIDES
from app.config.sources import CollectionMethod, SourceConfig

logger = logging.getLogger(__name__)


class CompanyBreachFetcher:
    """Fetch company breach news via RSS with HTML scraper fallback."""
    ...
    async def fetch(
        ...
        alt_feed = BREACH_FEED_OVERRIDES.get(source.id)
    ...
    def _prepare_source(self, source: SourceConfig) -> SourceConfig:
        feed = source.feed_url or BREACH_FEED_OVERRIDES.get(source.id)
        if feed:
    ...
    async def _safe_collect(
        ...
        try:
            ...
            logger.warning("Breach collector %s failed: %s", collector.source.id, exc)
