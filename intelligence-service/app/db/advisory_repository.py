...
class AdvisoryRepository:
    ...
    async def upsert(self, data: dict) -> VendorAdvisoryIntel | None:
        ch = data["content_hash"]
        existing = await self.session.execute(
            select(VendorAdvisoryIntel).where(VendorAdvisoryIntel.content_hash == ch)
        )
    ...
    async def list_advisories(
        ...
        limit: int = 50,
        offset: int = 0,
    ) -> list[VendorAdvisoryIntel]:
        query = (
            select(VendorAdvisoryIntel)
            .where(VendorAdvisoryIntel.confidence_score >= min_score)
            .order_by(
                VendorAdvisoryIntel.published_at.desc().nullslast(),
                VendorAdvisoryIntel.confidence_score.desc(),
            )
            .offset(offset)
            .limit(limit)
        )
        if source_id:
            query = query.where(VendorAdvisoryIntel.source_id == source_id)
        if vendor:
            query = query.where(VendorAdvisoryIntel.vendor == vendor)
    ...
    async def count(
        ...
        query = (
            select(func.count())
            .select_from(VendorAdvisoryIntel)
            .where(VendorAdvisoryIntel.confidence_score >= min_score)
        )
        if source_id:
            query = query.where(VendorAdvisoryIntel.source_id == source_id)
    ...
    async def get(self, advisory_id: str) -> VendorAdvisoryIntel | None:
        return await self.session.get(VendorAdvisoryIntel, advisory_id)
