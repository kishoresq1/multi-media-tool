...
from openosint.agent import GeminiAgent, OllamaAgent, OpenAICompatibleAgent, OpenOSINTAgent, OpenRouterAgent
...
_TOOL_INFO_ROWS = [
    ...
    ("search_dehashed", "DeHashed API", "Deep-web credential & password leak search"),
    ("search_feeds", "19 RSS sources", "AI-classified cybersecurity news feed"),
...
def _print_banner(provider: str, model: str) -> None:
    if provider == "ollama":
        ...
        provider_info = f"[dim]Provider: OpenRouter ({model})[/]"
    elif provider == "gemini":
        provider_info = f"[dim]Provider: Google Gemini ({model})[/]"
    else:
...
def _print_config(
    ...
    if provider == "anthropic":
        ...
        rows.append(f"[bold]Gateway:[/]  https://openrouter.ai/api/v1")
    elif provider == "gemini":
        rows.append(f"[bold]SDK:[/]      google-generativeai")
    else:
...
class OpenOSINTRepl:
    ...
    def __init__(
        ...
        self._agent: OpenOSINTAgent | OllamaAgent | OpenAICompatibleAgent | OpenRouterAgent | GeminiAgent
        if provider == "ollama":
            ...
            self._display_model = openrouter_model or os.environ.get("OPENROUTER_MODEL", "openai/gpt-4o-mini")
        elif provider == "gemini":
            self._agent = GeminiAgent(
                model=gemini_model,
                api_key=gemini_api_key,
            )
            self._display_model = gemini_model or os.environ.get("GEMINI_MODEL", "gemini-1.5-flash")
        else:
